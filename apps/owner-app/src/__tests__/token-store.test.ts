...
describe("tokenStore.getAccessToken", () => {
	it("returns the stored access token", () => {
		mockGetItem.mockReturnValue("owner-access-token");
		expect(tokenStore.getAccessToken()).toBe("owner-access-token");
...
describe("tokenStore.getRefreshToken", () => {
	it("reads from the refresh token key", () => {
		mockGetItem.mockReturnValue("owner-refresh-token");
		expect(tokenStore.getRefreshToken()).toBe("owner-refresh-token");
