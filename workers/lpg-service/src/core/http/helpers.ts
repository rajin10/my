...
// NOTE: a buggy `isNumber` helper used to live here (it tested `Number.isFinite`
// on a string and so always returned false). It was removed with its only caller
// in `query-parse.ts` — query scalars stay as strings and are coerced by the
// per-route Zod schemas. Don't reintroduce eager numeric coercion in the parser.
