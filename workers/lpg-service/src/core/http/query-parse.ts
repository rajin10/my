...
function parseValue(val: unknown) {
	if (typeof val === "undefined" || val === "") {
		...
		// A former `isNumber` → `Number()` branch here was dead (it tested
		// `Number.isFinite` on a string), so dropping it is behaviour-preserving.
		return val;
	}
}
 ...
 *
 * @param fields - Can be a comma-separated string, string array, or undefined
 * @returns Array of field strings, or undefined if fields is empty/undefined
 */
