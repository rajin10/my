...
export class UsersService {
	...
	async uploadPhoto(userId: string, file: File): Promise<{ url: string }> {
		// MIME allowlist + size cap; extension is derived from the validated content
		// type, never the user-supplied filename. Shared with the other photo paths.
		...
		if (oldUrl) {
			// photoUrl is `${PUBLIC_R2_URL}/${key}`. Strip any scheme + host to derive
			// the key — this also tolerates legacy scheme-less URLs from before the
			// PUBLIC_R2_URL scheme fix, which `new URL()` would have thrown on.
			const oldKey = oldUrl
				.replace(/^https?:\/\//, "")
				.split("/")
				.slice(1)
